...
function* newcreate(firstname, lastname, username, password) {
    try {
        const token = yield call(Api.newcreateUser, firstname, lastname, username, password);
...
export function* signupFlow() {
    while(true) {
        const {firstname, lastname, username, password} = yield take(actions.SIGNUP_ACTION);
